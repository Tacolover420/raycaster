...
var Dungeon = exports.Dungeon = function () {
  function Dungeon() {
    ...
    _util2.default.merge(this.opts, settings);

    this.grid = _util2.default.matrix(this.opts.rows, this.opts.cols, this.opts.wallTile);
    this.regions = _util2.default.matrix(this.opts.rows, this.opts.cols, null);
  ...
  _createClass(Dungeon, [{
    ...
    value: function inBounds(coords) {
      return _util2.default.inRange(coords.x, this.opts.rows) && _util2.default.inRange(coords.y, this.opts.cols);
    }
    ...
    value: function traverse() {
      ...
      if (arguments.length === 1) {
        topLeft = new _vector.Vector(0, 0);
        bottomRight = new _vector.Vector(this.opts.rows - 1, this.opts.cols - 1);
        f = arguments[0];
        ...
        throw RangeError('Dungeon.prototype.traverse either takes one function ' + 'or two vectors and a function as arguments.');
      }

      v = new _vector.Vector(0, 0);
    ...
    value: function generate() {
      if (this.generated) throw 'This Dungeon has already been generated.';
    ...
    key: 'placeRooms',
    value: function placeRooms() {
      var _this = this;

      var attempts = this.opts.roomPlacementAttempts;
      while (attempts > 0) {
        var topLeft = new _vector.Vector(_util2.default.randOdd(1, this.opts.rows - 2), _util2.default.randOdd(1, this.opts.cols - 2));

        var w = _util2.default.randOdd(this.opts.minRoomSize, this.opts.maxRoomSize);
        var h = _util2.default.randOdd(this.opts.minRoomSize, this.opts.maxRoomSize);

        var bottomRight = new _vector.Vector(topLeft.x + h - 1, topLeft.y + w - 1);
        var newRoom = new _room.Room(topLeft, bottomRight);

        var intersection = _util2.default.some(this.rooms, function (room) {
          return room.intersects(newRoom);
        });

        if (!intersection && this.inBounds(bottomRight)) {
          ++this.currentRegion;
          this.rooms.push(newRoom);
          this.traverse(newRoom.topLeft, newRoom.bottomRight, function (coords) {
            return _this.setFloorTile(coords, _this.opts.floorTile);
          });
        }
    ...
    key: 'spawnMaze',
    value: function spawnMaze(coords) {
      var _this2 = this;
      ...
      while (liveCells.length > 0) {
        c = _util2.default.last(liveCells);

        /* Directions in which adjacent tile is empty. */
        emptyDirs = _util2.default.filter(this.validDirectionsFrom(c, 2), function (dir) {
          return _this2.isEmptyAround(c.plus(dir.scaledBy(2)));
        });
        ...
        if (emptyDirs.length > 0) {
           ...
           * maze.
           */
          canGoSameDir = lastDir !== null && _util2.default.some(emptyDirs, function (dir) {
            return dir.eq(lastDir);
          });
          if (canGoSameDir && Math.random() > this.opts.curliness) {
            dir = lastDir;
          } else {
            dir = _util2.default.sample(emptyDirs);
          }

          if (this.inBounds(c.plus(dir))) this.setFloorTile(c.plus(dir), this.opts.floorTile);
    ...
    key: 'connectRegions',
    value: function connectRegions() {
      var connectorMap, connectors, r, c, coords, neighbours, regions, i, region, merged, independentRegions, connector, dest, sources, validConnectors;
        ...
        for (c = 1; c < this.opts.cols - 1; ++c) {
          ...
          if (this.tileAt(coords) === this.opts.wallTile) {
            ...
            for (i = 0; i < neighbours.length; ++i) {
              ...
              if (region !== null && !_util2.default.includes(regions, region)) regions.push(region);
            }
      ...
      while (independentRegions.length > 1) {
        ...
        regions = _util2.default.map(connectorMap[connector.toString()], function (region) {
        ...
        for (i = 0; i <= this.currentRegion; ++i) {
          if (_util2.default.includes(sources, merged[i])) {
            merged[i] = dest;
          }
        }

        independentRegions = _util2.default.reject(independentRegions, function (region) {
          return _util2.default.includes(sources, region);
        });

        /* Filter out connectors that no longer connect two or more regions. */
        validConnectors = _util2.default.filter(connectors, function (coords) {
          var regions = _util2.default.map(connectorMap[coords.toString()], function (region) {
            return merged[region];
          });
          var uniqueRegions = new Set(regions);
          if (uniqueRegions.size > 1) return true;
          if (Math.random() < this.opts.extraConnectorChance) this.setConnector(coords);
          return false;
        }.bind(this));
    ...
    key: 'eliminateDeadEnds',
    value: function eliminateDeadEnds() {
      var _this3 = this;
      ...
      while (!done) {
        ...
        for (r = 0; r < this.opts.rows; ++r) {
          for (c = 0; c < this.opts.cols; ++c) {
            coords = new _vector.Vector(r, c);
            if (this.tileAt(coords) !== this.opts.wallTile) {
              exits = _util2.default.filter(this.neighbours(coords), function (neighbour) {
                return !_this3.isWall(neighbour);
              });
              if (exits.length === 1) {
                done = false;
    ...
    key: 'setConnector',
    value: function setConnector(coords) {
      this.checkBounds(coords);
      this.grid[coords.x][coords.y] = this.opts.connectorTile;
    }
  }, {
    key: 'eraseTileAt',
    value: function eraseTileAt(coords) {
      this.checkBounds(coords);
    ...
    value: function checkBounds(coords) {
      if (!this.inBounds(coords)) {
        throw RangeError("This dungeon doesn't have a cell at (" + coords.x + ', ' + coords.y + ').');
      }
    }
  }, {
    key: 'checkGenerated',
    value: function checkGenerated() {
      if (!this.generated) throw 'Forgot to call Dungeon.generate()!';
    }
  }, {
    key: 'isEmptyAround',
    value: function isEmptyAround(coords) {
      var _this4 = this;

      return this.isWall(coords) && _util2.default.every(this.validDirectionsFrom(coords), function (d) {
        return _this4.isWall(coords.plus(d));
      });
    }
  }, {
    key: 'validDirectionsFrom',
    value: function validDirectionsFrom(coords) {
      var _this5 = this;

      var steps = arguments.length <= 1 || arguments[1] === undefined ? 1 : arguments[1];

      var directions = [new _vector.Vector(1, 0), /* north */
      new _vector.Vector(-1, 0), /* south */
      new _vector.Vector(0, -1), /* west */
      new _vector.Vector(0, 1)];
      /* east */
      return _util2.default.filter(directions, function (dir) {
        return _this5.inBounds(coords.plus(dir.scaledBy(steps)));
      });
    }
  }, {
    key: 'neighbours',
    value: function neighbours(coords) {
      var validDirections = this.validDirectionsFrom(coords);
      return _util2.default.map(validDirections, function (dir) {
        return coords.plus(dir);
      });
    }
  }]);
...
},{"./room":5,"./util":6,"./vector":7}],3:[function(require,module,exports){
'use strict';
...
var Game = exports.Game = function () {
  function Game(canvas, rows, cols) {
    ...
    this.map = new _dungeon.Dungeon({ rows: this.rows, cols: this.cols });

    this.viewedTiles = _util2.default.matrix(this.rows, this.cols, 0);
    ...
    this.dir = new _vector.Vector(-1, 0);
    /* Direction (and magnitude) of the camera's (orthogonal) plane */
    this.plane = new _vector.Vector(0, 0.66);
  }
  ...
  _createClass(Game, [{
    ...
    value: function setUp() {
      ...
      window.addEventListener('keydown', function (e) {
        _this.moveDirs[_this.keyCodeMap[e.keyCode]] = true;
      }, false);

      window.addEventListener('keyup', function (e) {
        _this.moveDirs[_this.keyCodeMap[e.keyCode]] = false;
      }, false);
    }
  }, {
    key: 'loop',
    ...
    key: 'walk',
    value: function walk(dir, t) {
      var newPos = this.pos.plus(dir.scaledBy(this.moveSpeed * t));
      if (this.map.inBounds(newPos) && !this.map.isWall(newPos.map(Math.floor))) {
        this.pos = newPos;
      }
    }
  }, {
    ...
    value: function render() {
      ...
      for (r = 0; r < this.viewedTiles.length; ++r) {
        for (c = 0; c < this.viewedTiles[0].length; ++c) {
          if (this.viewedTiles[r][c] !== 0 && !this.map.isWall(new _vector.Vector(r, c))) {
...
},{"./dungeon":2,"./util":6,"./vector":7}],4:[function(require,module,exports){
'use strict';

var _game = require('./game');

window.Game = _game.Game;
...
var Room = exports.Room = function () {
  function Room(topLeft, bottomRight) {
    _classCallCheck(this, Room);

    if (topLeft.x >= bottomRight.x || topLeft.y >= bottomRight.y) {
      throw RangeError('Top left corner of room must be above and to the ' + 'left of bottom right corner in grid.');
    }
  ...
  _createClass(Room, [{
    ...
    value: function intersects(other) {
       ...
       */
      this.bottomRight.x < other.topLeft.x || other.bottomRight.x < this.topLeft.x ||
      /* The index of the right-most column of one is less than the index of
       * the left-most column of the other.
       */
      this.bottomRight.y < other.topLeft.y || other.bottomRight.y < this.topLeft.y);
    }
  }]);
...
var Vector = exports.Vector = function () {
  ...
  _createClass(Vector, [{
    ...
    key: "projectedOnto",
    value: function projectedOnto(v) {
